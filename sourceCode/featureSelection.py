#Jake Felzien	
#I am using this file to test my algorithms with dimensionality reduction based on low variance of features
# this should remove a lot of data that perhaps i don't need 
			...
			if i not in countList:
				try:
					if abs(float(imgHash['img_'+str(i)+'.png']) - np.mean(outlierTotalList)) < m * np.std(outlierTotalList):
						...
						temp = loadImage(imgdirectory+'img_'+str(i)+'.png')
						temp.append(i)
